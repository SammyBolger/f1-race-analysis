...
class LeaderboardComponent(BaseComponent):
    def __init__(self, x: int, right_margin: int = 260, width: int = 240, visible=True, top_offset: int = 40):
        self.x = x
        self.width = width
        self.top_offset = top_offset  # Distance from top of window
        ...
        self.previous_positions = {}  # code -> previous position
    ...
    def draw(self, window):
        ...
        leaderboard_y = window.height - self.top_offset
        ...
        for i, (code, color, pos, progress_m) in enumerate(new_entries):
            ...
            # Position change indicator (up/down arrows)
            prev_pos = self.previous_positions.get(code)
            if prev_pos is not None and prev_pos != current_pos:
                pos_diff = prev_pos - current_pos  # positive = gained positions
                if pos_diff > 0:
        ...
        # Update previous positions for next frame comparison (every 60 frames ~1 second)
        # Initialize on first draw to avoid delayed indicators
        current_frame = getattr(window, 'frame_index', 0)
        if self.last_update_frame == -1:
            for i, (code, _, _, _) in enumerate(new_entries):
                self.previous_positions[code] = i + 1
            self.last_update_frame = int(current_frame)
        elif int(current_frame) - self.last_update_frame >= 60:
            for i, (code, _, _, _) in enumerate(new_entries):
                self.previous_positions[code] = i + 1
            self.last_update_frame = int(current_frame)
...
class LapTimeLeaderboardComponent(BaseComponent):
    def __init__(self, x: int, right_margin: int = 260, width: int = 240, top_offset: int = 40):
        self.x = x
        self.width = width
        self.top_offset = top_offset  # Distance from top of window
